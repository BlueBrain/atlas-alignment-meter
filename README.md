...
To run it only on a selected subset of region IDs, use `-r` or `--regions` and a comma-separated list (with no whitespace between them)
```
...
```
Note that the `-t` options followed by a number runs the CLI on the given number of threads. If not provided or providing `-t AUTO`, then the CLI runs on *(max_number_of_thread - 1)* to not bloat the machine. Keep in mind that running a process on more threads than physically available will perform poorly.
...
Some imagery capture methods rely on slicing a brain mechanically, capturing a picture of each slice, and later reconstructing the volume from slices digitally stuck together in the correct order. One drawback of this method is the slight displacement of each slice to the next, resulting in volume being imperfectly aligned along the axis orthogonal to the slicing plane.
...
At Blue Brain, we also have some procedures to obtain a smooth volume from a jagged one, to place cells or build circuits. Hence, it is key to provide a metric to evaluate the efficiency of the re-alignment process and eventually improve it.
...
> "What is the ratio of ROI voxels that are different from one slice to the next over the total count of ROI voxels in these two slices?"
...
The cons of this method (and how they have worked around) are:
- The slice just before the beginning of a region or the slice just after the end will have `0` voxel of this ROI, hence the ratio is always going to be `1.0`. Furthermore, a ratio of `1.0` can happen only in this particular case. Since we do not want to measure where an ROI is starting or ending but only want to measure the jaggedness happening within an ROI, we can just safely remove all the `1.0` from our set of measures. The reason for removing those values is that a ratio of `1.0` would add a bias to the final measures, making an ROI look more jagged than it is.
- The method is based on a difference from a slice to the next, as a result, it will not be possible to compute this value for the very last slice of the volume. The solution is to simply not compute the ratio on the last slice and making it *nullish*.

**Note 1:** the *no data* part around the brain is *NOT* considered as an ROI and its score is not computed.  

**Note 2:** The final score, from a pair of slices or averaged on a whole volume, will not tell whether or not a volume has an acceptable jaggedness, as this entirely depends on the conducted experiment and the eye of the scientist. To compensate for this lack of objectivity, the best course of action is to run this method on a reference volume, either an unacceptably jagged one or a very smooth one (or both). This will minimize the interval of confidence and help bear a judgment on a candidate volume. 

## Part 1: The per-region metrics
It may happen that an annotation volume is aligned with an algorithm that takes into consideration mostly its external envelope and does not process the alignment on internal structure. As a result, the whole brain mask would be fairly smooth but the internal parts would still be very much jagged.  

The proposed method adopts a per-parcellation tactic, creating an ROI (binary mask) for all the unique regions present in an annotation volume. Then, for each ROI mask, a slice-to-slice approach is ran.  
...
**Important notice:** some slices may not be populated by any ROI, or be populated by less ROI than other slices. In order to avoid a *no data bias* that would artificially lower the score (and make slices less jagged than they are), the per-slice approach is only taking into account the ROIs that are present on this particular slice index, discarding the `null` of the other ROIs that not present. (in other words, the absents are not part of the computed statistics).

## Part 3: The global metrics
Comparably to the *per-slice* approach, the metrics related to the global score (`mean`, `std`, `median`, `min` and `max`) are computed after discarding all the *no data* of outside the brain and the *no data* around each ROI.  
As a general comment, the global metrics are best suited for a not-too-in-depth sneak peek into the jaggedness metrics.

# Output
A Python dictionary is output and can be saved as JSON. Samples of these JSON files can be found in the folder `test_data/*.json`, where they are related to the volumes of the same name `*.nrrd`.
...
Following this logic, we can say that the ML-aligned candidate is even jaggier than the original CCF v2, which is the opposite of its original intention.
...
Then, looking at datasets such as AIBS Mouse CCF v3 volume in 25um (no rotation), we know that those axes, in respective order, corresponds to:
